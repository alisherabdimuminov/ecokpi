...
const addUser = async () => {
    ...
    let response = await $fetch<IResponse<{ errors: string[] }>>(apify("admin/users/add"), {
        method: "POST",
        body: form,
        headers: {
...
<template>
    <div class="flex flex-col gap-5 p-5 md:p-10">
        ...
        <div class="flex justify-between">
            <div></div>
            <div>
                <Button @click="addUser">{{ t('save') }}</Button>
            </div>
        </div>
        <div class="flex flex-col md:flex-row gap-3">
            ...
            <Card class="w-ful md:w-64 h-66 bg-accent/30">
                <CardHeader>
                    <CardTitle>Rasmi</CardTitle>
                </CardHeader>
                <CardContent class="flex flex-col items-center gap-2">
                    <img class="w-56 h-56 rounded-md" src="" alt="">
