<script setup lang="ts">


const { $t } = useLang();
</script>

<template>
    <div class="flex flex-col gap-5 p-5 md:p-10">
        <ClientOnly>
            <Breadcrumb>
                <BreadcrumbList>
                    <BreadcrumbItem>
                        <BreadcrumbLink>
                            <NuxtLink :to="{ name: 'index' }">{{ $t('home') }}</NuxtLink>
                        </BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbLink>
                            <NuxtLink :to="{ name: 'users' }">{{ $t('users') }}</NuxtLink>
                        </BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbPage>
                            {{ $t('add') }}
                        </BreadcrumbPage>
                    </BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>
        </ClientOnly>

        <div class="flex flex-col md:flex-row gap-3">
            <Card class="w-full bg-accent/30">
                <CardHeader>
                    <CardTitle>Ma'lumotlar</CardTitle>
                </CardHeader>
                <CardContent>
                    <Label>ID</Label>
                    <Input />

                    <Label>Foydalanuvchi nomi</Label>
                    <Input />

                    <Label>Familiya Ism Sharifi</Label>
                    <Input />
                </CardContent>
            </Card>
            <Card class="w-full md:w-72 h-66 bg-accent/30">
                <CardHeader>
                    <CardTitle>Rasmi</CardTitle>
                </CardHeader>
                <CardContent class="flex flex-col gap-1">
                    <img class="h-56 md:h-48 w-full md:w-56 rounded-md" src="data:image/png;base64," alt="">
                    <Input type="file" />
                </CardContent>
            </Card>
        </div>
        <Card class="w-ful bg-accent/30">
            <CardHeader>
                <CardTitle>Rasmi</CardTitle>
            </CardHeader>
            <CardContent>
                <div class="flex flex-col md:flex-row gap-3">
                    <div class="w-full">
                        <Label>Passport raqami</Label>
                        <Input />
                    </div>
                    <div class="w-full">
                        <Label>JShShR</Label>
                        <Input />
                    </div>
                </div>

                <div class="flex flex-col md:flex-row gap-3">
                    <div class="w-full">
                        <Label>Filial</Label>
                        <Select>
                            <SelectTrigger>
                                <SelectValue placeholder="Filialni tanlang" />
                            </SelectTrigger>
                        </Select>
                    </div>
                    <div class="w-full">
                        <Label>Bo'lim</Label>
                        <Select>
                            <SelectTrigger>
                                <SelectValue placeholder="Bo'limni tanlang" />
                            </SelectTrigger>
                        </Select>
                    </div>
                </div>
            </CardContent>
        </Card>

        <Card class="bg-accent/30">
            <CardHeader>
                <CardTitle>Vazifalar</CardTitle>
            </CardHeader>
            <CardContent>
                <div class="border rounded-md overflow-auto">
                    <Table>
                        <TableHeader class="border-b">
                            <TableHead>ID</TableHead>
                            <TableHead class="border-l">Vazifa nomi</TableHead>
                            <TableHead class="whitespace-nowrap border-l">Vazifa muddati</TableHead>
                            <TableHead class="whitespace-nowrap border-l">Ball</TableHead>
                        </TableHeader>
                        <TableBody>
                            <TableRow>
                                <TableCell class="w-4">1</TableCell>
                                <TableCell class="border-l">Ijro intizomi</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">Doimiy</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">
                                    <div class="h-8 w-8 flex items-center justify-center text-green-500 border border-green-500 rounded-full p-1">
                                        20
                                    </div>
                                </TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell class="w-4">2</TableCell>
                                <TableCell class="border-l">Mahnat intizomiga riyo etilganligi</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">Doimiy</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">
                                    <div class="h-8 w-8 flex items-center justify-center text-green-500 border border-green-500 rounded-full p-1">
                                        7
                                    </div>
                                </TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell class="w-4">3</TableCell>
                                <TableCell class="border-l">Odob ahloq intizomiga riyo etilganligi</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">Oylik</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">
                                    <div class="h-8 w-8 flex items-center justify-center text-green-500 border border-green-500 rounded-full p-1">
                                        3
                                    </div>
                                </TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell class="w-4">4</TableCell>
                                <TableCell class="border-l">Tashabusskorlik</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">Oylik</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">
                                    <div class="h-8 w-8 flex items-center justify-center text-green-500 border border-green-500 rounded-full p-1">
                                        10
                                    </div>
                                </TableCell>
                            </TableRow>
                        </TableBody>
                    </Table>
                </div>
            </CardContent>
        </Card>

        <Card class="bg-accent/30">
            <CardHeader>
                <CardTitle class="flex items-center justify-between">
                    <p>Maxsus vazifalar</p>
                    <Dialog>
                        <DialogTrigger>
                            <Button>Qo'shish</Button>
                        </DialogTrigger>
                        <DialogContent>
                            <DialogHeader>
                                <DialogTitle>Yangi vazifa qo'shish</DialogTitle>
                            </DialogHeader>
                            
                            <Label>Vazifa</Label>
                            <Input />

                            <Label>Muddati</Label>
                            <Select>
                                <SelectTrigger>
                                    <SelectValue placeholder="Muddatni tanlang"/>
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="doimiy">Doimiy</SelectItem>
                                    <SelectItem value="oylik">Oylik</SelectItem>
                                    <SelectItem value="yillik">Yillik</SelectItem>
                                </SelectContent>
                            </Select>

                            <Label>Ball</Label>
                            <NumberField>
                                <NumberFieldContent>
                                    <NumberFieldDecrement />
                                    <NumberFieldInput />
                                    <NumberFieldIncrement />
                                </NumberFieldContent>
                            </NumberField>

                            <DialogFooter>
                                <DialogClose>
                                    <Button variant="destructive">Yopish</Button>
                                </DialogClose>
                                <Button>Qo'shish</Button>
                            </DialogFooter>
                        </DialogContent>
                    </Dialog>
                </CardTitle>
            </CardHeader>
            <CardContent>
                <div class="border rounded-md overflow-auto">
                    <Table>
                        <TableHeader class="border-b">
                            <TableHead>ID</TableHead>
                            <TableHead class="border-l">Vazifa nomi</TableHead>
                            <TableHead class="whitespace-nowrap border-l">Vazifa muddati</TableHead>
                            <TableHead class="whitespace-nowrap border-l">Ball</TableHead>
                        </TableHeader>
                        <TableBody>
                            <TableRow>
                                <TableCell class="w-4">1</TableCell>
                                <TableCell class="border-l">Ijro intizomi</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">Doimiy</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">
                                    <div class="h-8 w-8 flex items-center justify-center text-green-500 border border-green-500 rounded-full p-1">
                                        20
                                    </div>
                                </TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell class="w-4">2</TableCell>
                                <TableCell class="border-l">Mahnat intizomiga riyo etilganligi</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">Doimiy</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">
                                    <div class="h-8 w-8 flex items-center justify-center text-green-500 border border-green-500 rounded-full p-1">
                                        7
                                    </div>
                                </TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell class="w-4">3</TableCell>
                                <TableCell class="border-l">Odob ahloq intizomiga riyo etilganligi</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">Oylik</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">
                                    <div class="h-8 w-8 flex items-center justify-center text-green-500 border border-green-500 rounded-full p-1">
                                        3
                                    </div>
                                </TableCell>
                            </TableRow>
                            <TableRow>
                                <TableCell class="w-4">4</TableCell>
                                <TableCell class="border-l">Tashabusskorlik</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">Oylik</TableCell>
                                <TableCell class="w-4 whitespace-nowrap border-l">
                                    <div class="h-8 w-8 flex items-center justify-center text-green-500 border border-green-500 rounded-full p-1">
                                        10
                                    </div>
                                </TableCell>
                            </TableRow>
                        </TableBody>
                    </Table>
                </div>
            </CardContent>
        </Card>
    </div>
</template>
